...
func newMutexes() *mutexes {
		...
		for {
			...
			case <-m.purge:
				...
				for key, item := range m.items {
					if time.Since(item.lastAccess) > config.MutexStaleMutexes ||
						len(m.items) > config.MutexMaxCacheSize && item.locks == 0 {
	...
	go func() {
		for {
			time.Sleep(config.MutexCleanupFrequency)
			m.purge <- struct{}{}
		}
	}()
...
func (m *mutexes) getItem(key interface{}) *mutexItem {
	...
	if !ok {
		...
		if len(m.items) > config.MutexMaxCacheSize {
			m.purge <- struct{}{}
		}
	}
